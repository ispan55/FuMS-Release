...
waitUntil {  diag_fps > FuMS_FPSMinimum};
_hc publicVariableClient "FuMS_ListofMissions";// Array of ["Name", "string of preprocessed code"] at each theme index.
{ 
    waitUntil {  diag_fps > FuMS_FPSMinimum};
...
{ 
    _curFPS = diag_fps;
	waitUntil {  (_curFPS  > _startFPS) and (_curFPS > FuMS_FPSMinimum)};
    
    _codeString = missionNamespace getVariable _x;
    diag_log format ["##InitHeadlessClient: ---transmitting %1 to HC:%2    :",_x,_hc];
