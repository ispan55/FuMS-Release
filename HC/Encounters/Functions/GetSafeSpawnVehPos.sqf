...
FindNearestRoad = compile preprocessFileLineNumbers "HC\Encounters\Functions\FindNearestRoad.sqf";
FindNearestWater = compile preprocessFileLineNumbers "HC\Encounters\Functions\FindNearestWater.sqf";
FindNearestLand = compile preprocessFileLineNumbers "HC\Encounters\Functions\FindNearestLand.sqf";
...
if (_veh isKindOf "Boat") then {_pos = [_pos] call FindNearestWater;}
else
{           
    if (TypeName _driver == "ARRAY") then
    {
        ...
        { 
            ...
            _pos = [_pos] call FindNearestLand;
        };
        if (_aiLogic == "CONVOY" and _veh isKindOf "Land") then
        {
            if(_aiOptions select 2) then
            {
                ...
                _pos = [_pos] call FindNearestRoad;
            };
        };
