...
if (!isNil "_offset") then
{
    ...
    if (typeName _offset == "STRING") then { _newloc = [_offset] call FindTown;}  // just a string "Stavros"
    else
    {  
...
}else
{
    ...
    if (typeName _origin == "STRING") then { _newloc = [_origin] call FindTown;}  // just a string "Stavros"
