...
_themeRadioData = THEMEDATA select _index;
...
_numgroups = GroupCount select _index; // get number groups made so far for this theme.
_numgroups = _numgroups +1;
GroupCount set [_index, _numgroups];
...
if (_aiDeathMsg) then
{
  ...
  //  diag_log format ["##AIRadio: Killed EH added for %1",_callsign];
    ...
    _units = units _group;
    {
        ...
        _x setVariable ["RadioChat", [_unitCallsign,_radioChannel,_radioRange,_index     ], false];
        _unitnum = _unitnum +1;      
    }foreach _units;
};
...
{ 
    ...
    {
        AI_RCV_MsgQue set [ _index, [_callsign,"CheckIn"]]; 
    };
};
...
while {alive _leader} do
{
 ...
 //   diag_log format ["##AIRadio: _initial:%1, _cur:%2, _prev:%3",_initialGroupSize, _curGroupSize, _prevGroupSize];
    ...
    _queMsg = (AI_RCV_MsgQue select _index) select 0;
    if (  _queMsg==_callsign or _queMsg==_aiCallsign) then // if =_aiCallsign, message intended for ALL groups!
    {
      //  diag_log format ["##AIRadio: RCV_MsqQue match:  %1",_queMsg];
        {    // Response loop.       
            if ( ( (AI_RCV_MsgQue select _index) select 1) == (_x select 0) )then
            {
                ...
                [_callsign, _baseCallsign, _x select 1, _radioChannel, _radioRange, _pos, 
                     [_leader, _missionName, _curGroupSize, _initialGroupSize]  ] call FormatAndBroadcastAI;
    ...
    { 
        AI_XMT_MsgQue set [_index, [_callsign, "Death"]]; 
    ...
    { 
        if (time > _detTime+600) then
        {
            ...
            {
                _detTime = time;
                AI_XMT_MsgQue set [_index, [_callsign, "Detected"]];
            ...
            }else
            {
               ...
               _detTime = time;
                AI_XMT_MsgQue set [_index, [_callsign, "DetHostileAI"]];
    ...
    { 
        if (time > _helpTime+180) then
        {
            ...
            {
                AI_XMT_MsgQue set [_index, [_callsign, "Less50"]];
                ...
                RC_REINFORCEMENTS = [_pos, _index, _callsign];               
            };
        };
    };
};
