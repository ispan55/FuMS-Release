...
if (!_abort) then
{
    { 
        ...
        for [{_i=0},{_i < _numVeh},{_i=_i+1}] do
        {
            ...
            _mode = "";
            _pos = [_eCenter, _vehLoc select _i] call XPos;
            _driver = "none";
            if (_i < _numdrivers) then {_driver = _driverIndividual select _i;};
            _data = [_pos, _driver, _vehType select _i] call GetSafeSpawnVehPos;	
            //diag_log format ["pos:%1, driver:%2, type:%3 data:%4",_pos, _driver, _vehType select _i,_data];
            _veh = [ _vehType select _i, _data select 0, [], 30 , _data select 1] call HC_CreateVehicle;		
        ...
        { 
            ...
            if (! ( (_vehicles select _i) isKindOf "Air") ) then {[_drivers select _i] execVM "HC\Encounters\AI_Logic\VehStuck.sqf";};
        };              
        ...
        for [{_i=0},{_i < count _vehCrew},{_i=_i+1}] do
        {
            ...
            if (!isNil "_crewData") then // vehicle has crew!
            {
                ...
                {
                    _turretsArray =[_vehicles select _i] call KK_fnc_commonTurrets;
                    ...
                    for [{_ii =0}, {_ii < _crewcount}, {_ii = _ii+1}] do
                    {
                        ...
                        {    
                            ...
                            _crew = [group _leader,_type, getPos _crewVeh, _themeIndex] call SpawnSoldier;                   
