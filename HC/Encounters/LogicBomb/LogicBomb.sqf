...
// If 'NO TRIGGERS' skip trigger initialization!
{
       ...
       // diag_log format ["##Trigger Set:State:%1 :: %2 initializing", _state,_x];
        {
            ...
            switch (_trigName) do
            {
                case "PROXPLAYER":
                {
                    ...
                    _pos = [_eCenter, _offset] call XPos;
    ...
    HC_HAL_TRIGGERS = HC_HAL_TRIGGERS + [_instanceTriggers];
    publicVariableServer "HC_HAL_TRIGGERS";    
    ...
    while {_msnStatus != "WIN" and _msnStatus !="LOSE"} do
    {
        ...
        for [{_i=0},{_i < 5},{_i = _i+1}] do // go through each of the states checking status of their triggers
        {
            ...
            if (_stateMax select _i > 0 ) then // There are some triggers for this state.
            {
                //******************************
                //_proxPlayer [pos, range, numPlayers] 
                _triggerCount = _triggerCount + ([(_triggerArray select 0) select _i,_i] call ProxPlayer);               
                //***********************************
                //_lowUnitCount [trigger, numAI]
                _triggerCount = _triggerCount + ([(_triggerArray select 1) select _i,_i,"LOW"] call UnitCount);
                //***********************************
                //_highUnitCount [trigger, numAI] 
                _triggerCount = _triggerCount + ([(_triggerArray select 2) select _i,_i,"HIGH"] call UnitCount);
                //*************************************
                // _detected [group, vehicle]
                _triggerCount = _triggerCount + ([(_triggerArray select 3) select _i,_groups, _vehicles, _i] call Detect);
                //**************************************
                // _timer [expirationTime]
                _triggerCount = _triggerCount  + ([(_triggerArray select 4) select _i, _i] call Timer);
                //***************************************
                // _bodyCount [numAI, range]
                _triggerCount = _triggerCount + ( [(_triggerArray select 5) select _i, _i] call GetBodyCount);
                // _allDeadorGone [themeIndex]
                _triggerCount = _triggerCount + ( [(_triggerArray select 6) select _i, _i] call AllDeadorGone);
                    ...
                    case 2: 
                    {
                       ...
                       diag_log format ["##LogicBomb: PHASE TRANSITION: %3: Starting %1:%2",_phase01ID, _phase01,_curMission];
                        ...
                        if (typeName _phase01 == "ARRAY") then
                        {
                            ...
                            _msnPhaseLoc = [_eCenter, _phase01 select 1] call XPos;
                        }else
                        {
                    ...
                    case 3:
                    {
                       ...
                       diag_log format ["##LogicBomb: PHASE TRANSITION: %3: Starting %1:%2",_phase02ID, _phase02,_curMission];
                          ...
                          if (typeName _phase02 == "ARRAY") then
                        {
                            ...
                            _msnPhaseLoc = [_eCenter, _phase02 select 1] call XPos;
                        }else
                        {
                    ...
                    case 4:
                    {
                       ...
                       diag_log format ["##LogicBomb: PHASE TRANSITION: %3: Starting %1:%2",_phase03ID, _phase03,_curMission];
                          ...
                          if (typeName _phase03 == "ARRAY") then
                        {
                            ...
                            _msnPhaseLoc = [_eCenter, _phase03 select 1] call XPos;
                        }else
                        {
